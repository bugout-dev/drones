...
from brood.db import BROOD_DB_URI
from brood.db import SessionLocal as session_local_brood
from brood.db import create_brood_engine
from brood.models import Token, TokenType, User
...
def tokens_cleanup_handler(args: argparse.Namespace) -> None:
    """
    Clean old tokens with params:

    token_type: bugout
    created_at: <= max-lifetime
    autogenerated user: false
    restricted: false
    """
    custom_engine = create_brood_engine(
        url=BROOD_DB_URI,
        pool_size=BUGOUT_SPIRE_THREAD_DB_POOL_SIZE,
        statement_timeout=300000,
    )

    today_diff = datetime.today() - timedelta(days=args.max_lifetime)
    try:
        with yield_session_maker(engine=custom_engine) as db_session:
            # Filter autogenerated users should exist as additional query,
            # otherwise join not supports sqlalchemy .delete(synchronize_session=False)
            real_users = db_session.query(User.id).filter(User.autogenerated == False)
            old_tokens = (
                db_session.query(Token)
                .filter(Token.user_id.in_(real_users))
                .filter(Token.token_type == TokenType.bugout)
                .filter(Token.restricted == False)
                .filter(Token.created_at <= today_diff)
                .delete(synchronize_session=False)
            )
            logger.info(f"Amount of deleted old tokens: {old_tokens}")
            db_session.commit()
    except Exception as err:
        logger.error(f"Unable to clean old tokens, error: {str(err)}")
...
def main() -> None:
    ...
    parser_cleanup_tokens = subcommands_cleanup.add_parser(
        "tokens", description="Clean up tokens"
    )
    parser_cleanup_tokens.add_argument(
        "--max-lifetime",
        type=int,
        required=True,
        help="Max lifetime of token in days",
    )
    parser_cleanup_tokens.set_defaults(func=tokens_cleanup_handler)
